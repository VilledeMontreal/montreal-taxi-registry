...
export class MigrateScript extends ScriptBase<IMigrationArguments> {
  ...
  protected async configure(command: Command): Promise<void> {
    command.argument(
      "<databaseType>",
      `${DatabaseType.mongodb} or ${DatabaseType.postgresql}`
    );
  ...
  protected async main() {
    const { databaseType, databaseVersion, options } = this
      .args as IMigrationArguments;
  ...
  protected async executeMigration(
    ...
    if (
      !Object.keys(migrationsByDatabaseTypeAndVersion[databaseType]).includes(
        databaseVersion
      )
    ) {
      throw new Error(
        `The database type: ${databaseType} version: ${databaseVersion} is invalid`
      );
    }

    const databaseTypeMigrations =
      migrationsByDatabaseTypeAndVersion[databaseType];
