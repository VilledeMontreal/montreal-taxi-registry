...
export class Configs {
  ...
  get api() {
    return {
       ...
       */
      domainPath: this.cache.get<string>(
        "api.domainPath",
  ...
  get routing() {
    return {
       ...
       */
      maxRequestSizeMb: this.cache.get<number>(
        "routing.maxRequestSizeMb",
      ...
      routes: {
        openAPI: {
          specsFile: this.cache.get<string>(
            "routing.routes.openAPI.specsFile",
            (rawVal: string): string => {
              const reVal = rawVal;
              return _.trim(reVal, "/");
            }
