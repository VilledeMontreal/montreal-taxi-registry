...
class TaxiEstimateAccuracyRepository {
  public async insertTestExecution({
    ...
    try {
      const responsePersistedTestExecution =
        await taxiEstimatePostgrePool.query(selectInsertedTestExecution, [
          testExecutionId,
        ]);
  ...
  public async insertEstimatedTrips(
    ...
    try {
      ...
      await taxiEstimatePostgrePool.query(insertEstimatedTrips, [
        JSON.stringify(estimatedTrips),
      ]);
    } catch (error) {
      throw new Error(
  ...
  public async getRealTripsCount(sampleId: number): Promise<number> {
    try {
      const response = await taxiEstimatePostgrePool.query(countRealTrips, [
        sampleId,
      ]);
      handleEmptyResponse(response);
...
export const taxiEstimateAccuracyRepository =
  new TaxiEstimateAccuracyRepository();
