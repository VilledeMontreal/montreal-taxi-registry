...
class UserRepository {
  ...
  public async getUserByApiKey(apikey: string): Promise<UserModel> {
    ...
    const queryResult = await postgrePool.query<UserModel>(getUserByApikey, [
      cipher,
    ]);
    if (!queryResult || !queryResult.rows || !queryResult.rows[0]) {
      return null;
  ...
  public async getUsersPaginated(
    ...
    const queryResult = await postgrePool.query<UserModel>(query, [
      pageNumber,
      pageSize,
    ]);
  ...
  private async setUserRole(
    ...
    if (!canChangeUserRole)
      throw new UnauthorizedError("Changing role not allowed");
  ...
  private async canChangeUserRole(
    ...
    return (
      ...
      (originalRoleName === UserRole.Manager &&
        desiredRoleName !== UserRole.Admin)
    );
  }
}
