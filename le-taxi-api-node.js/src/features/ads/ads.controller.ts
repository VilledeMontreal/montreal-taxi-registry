...
class AdsController {
  ...
  public async upsertAds(request: Request, response: Response) {
    const adsRequestDto = await validateAdsRequest(request, new AdsRequestDto());
    const upsertedAds = await adsDataAccessLayer.upsertAds(adsRequestDto, request.userModel);
    const adsResponseDto = await adsDataAccessLayer.getAdsById(Number(upsertedAds.entityId));
