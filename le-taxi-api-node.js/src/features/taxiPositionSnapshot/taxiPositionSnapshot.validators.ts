...
import { isLatitudeValid, isLongitudeValid, isNumber } from '../shared/validations/validators';
...
export function validateTaxiPositionSnapshot(taxiPositionSnapshot: any, currentUser: UserModel): void {
  const validationErrorMessages: string[] = [];
  const isInvalidSnapshotJsonFormat =
    !taxiPositionSnapshot || !Array.isArray(taxiPositionSnapshot.items) || taxiPositionSnapshot.items.length === 0;
  if (isInvalidSnapshotJsonFormat) {
    validationErrorMessages.push('the taxi position snapshot is not in a valid JSON format');
  } else {
    taxiPositionSnapshot.items.forEach((snapshot: any) => {
      validationErrorMessages.push(...validateTaxiPositionSnapShotItem(snapshot, currentUser));
    });
  }
...
function validateTaxiPositionSnapShotItem(
  ...
  if (!hasValidOperator(snapshotItem.operator, currentUser)) {
    validationErrorMessages.push('the operator is either missing or has an invalid API key');
  }
  ...
  if (!snapshotItem.speed || isNaN(snapshotItem.speed)) {
    validationErrorMessages.push('the speed attribute is mandatory and must be a number');
  }
...
export async function validateTaxiOwnership(taxiPositionSnapshot: TaxiPositionSnapshotRequestDto, operatorId: string) {
