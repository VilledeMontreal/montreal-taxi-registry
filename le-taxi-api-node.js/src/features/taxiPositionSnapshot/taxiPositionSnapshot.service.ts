...
class TaxiPositionSnapshotService {
  ...
  private async persistIndexInMongoDb(taxiPositionSnapshot: TaxiPositionSnapshotRequestDto, userModel: UserModel) {
    const taxiSummaryReferences = await taxiSummaryRepositoryWithCaching.getByKeys(
      taxiPositionSnapshot.items.map(item => item.taxi)
    );
    const latestTaxiPosition = latestTaxiPositionMapper.toLatestTaxiPositionModels(
      taxiSummaryReferences,
      taxiPositionSnapshot,
      userModel
    );
    await latestTaxiPositionRepository.saveLatestTaxiPositions(latestTaxiPosition);
  }

  private async persistHistoryInMongoDb(taxiPositionSnapshot: TaxiPositionSnapshotRequestDto): Promise<void> {
    const db = mongoUtilities.getMongoDb();

    const taxiPositionSnapshotToPersist: TaxiPositionSnapshotRequestDto = {
      items: taxiPositionSnapshot.items.filter(snapshot => snapshot.status !== TaxiStatus.Off),
      receivedAt: taxiPositionSnapshot.receivedAt
    };

    await db.collection('taxiPositionSnapshots').insertOne(taxiPositionSnapshotToPersist);
  }
}
