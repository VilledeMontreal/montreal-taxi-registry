...
export class TripBuilder {
  ...
  public parseTaxiPositionSnapshot(taxiPositionSnapshot: TaxiPositionSnapshotRequestDto): void {
    taxiPositionSnapshot.items.forEach(taxiPosition =>
      this.parseTaxiPosition(taxiPosition, taxiPositionSnapshot.receivedAt.toISOString())
    );
  }
  ...
  public deleteCompletedTrips(): void {
    Object.values(this._tripParserByTaxiId).forEach(tripBuilder => tripBuilder.deleteCompletedTrips());
  }

  private parseTaxiPosition(taxiPosition: TaxiPositionSnapshotItemRequestDto, receivedAt: string): void {
    if (!this._tripParserByTaxiId[taxiPosition.taxi]) {
      this._tripParserByTaxiId[taxiPosition.taxi] = new TripParser(taxiPosition.taxi);
    }

    this._tripParserByTaxiId[taxiPosition.taxi].parseTaxiPosition(taxiPosition, receivedAt);
  }
}
