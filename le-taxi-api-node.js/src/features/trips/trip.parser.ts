...
export class TripParser {
  ...
  public parseTaxiPosition(
    ...
    if (this._lastStatus) {
      if (
        this._lastStatus !== TaxiStatus.Occupied &&
        taxiPosition.status === TaxiStatus.Occupied
      ) {
  ...
  private tripInProgress(receivedAt: string): boolean {
    return (
      !!this._currentTrip && this._currentTrip.departureTime !== receivedAt
    );
  }
  ...
  private getTripScore(): [ConfidenceScore, string] {
    if (
      this._currentTrip.distanceMeters <
      tripParserConfig.distanceLowerLimitInMeter
    ) {
    ...
    if (
      this._currentTrip.distanceMeters >
      tripParserConfig.distanceUpperLimitInMeter
    ) {
      return [ConfidenceScore.Low, "DistanceGreaterThanLimit"];
    }

    if (
      this._currentTrip.totalDurationSeconds <
      tripParserConfig.durationLowerLimitInSeconds
    ) {
    ...
    if (
      this._currentTrip.totalDurationSeconds >
      tripParserConfig.durationUpperLimitInSeconds
    ) {
      return [ConfidenceScore.Low, "DurationLongerThanLimit"];
    }

    const averageSpeed =
      this._currentTrip.distanceMeters / this._currentTrip.totalDurationSeconds;
    ...
    if (this._currentTrip.confidence)
      return [this._currentTrip.confidence, this._currentTrip.confidenceReason];

    const idlePercentage =
      (this._currentTrip.idleDurationSeconds /
        this._currentTrip.totalDurationSeconds) *
      100;
    if (idlePercentage < 15) return [ConfidenceScore.High, null];

    if (idlePercentage < 30)
      return [ConfidenceScore.Medium, "IdlePercentageBetween15And30"];
...
function asLineString(positions: number[][]): string {
  ...
  if (positions.length === 1)
    return `POINT (${positions[0][0]} ${positions[0][1]})`;
