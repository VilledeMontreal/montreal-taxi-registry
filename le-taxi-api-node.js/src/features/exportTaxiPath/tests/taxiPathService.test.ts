...
import { expectedGeoJsonPayload, geoPositionsSample, taxiInfoSample, taxiSnapshotSample } from './testData';
...
describe('TaxiPathService TESTS', () => {
  ...
  context('convertDataToGeoJson', () => {
    it('should return data in geojson format', () => {
      ...
      const geoJson = taxiPathService.generateGeoJson(geoLineString, taxiSnapshotSample);
  ...
  context('getPositionFromData', () => {
    it('should return positions from taxi position snapshots', () => {
      const positions = taxiPathService.getPositionsFromSnapshots(taxiSnapshotSample);
