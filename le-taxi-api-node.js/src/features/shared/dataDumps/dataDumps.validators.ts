...
function validateDate(request: Request): string {
  try {
    ...
    throw new BadRequestError('Invalid date format received. Ex:YYYY-MM-DDThh:mm:ss.nnnZ');
  }
}

function validateEncoding(request: Request): Request {
  if (request.header('accept-encoding')?.toLowerCase() !== 'gzip') {
    throw new BadRequestError(`Invalid header. 'Accept-Encoding' header must be set to 'gzip'`);
  }
  return request;
}
