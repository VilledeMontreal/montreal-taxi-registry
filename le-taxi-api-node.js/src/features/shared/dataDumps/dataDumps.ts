...
async function getDateFromSql(table: string, column: string, operator?: string) {
  const result = operator ? await queryDateWithOperator(table, column, operator) : await queryDate(table, column);
...
async function buildDataDumpStream(selectAll: string, operator?: string): Promise<any> {
...
async function queryDate(table: string, column: string): Promise<QueryResult<any>> {
  const query = `SELECT max(${column}) as lastDate FROM ${table}`;
  return await postgrePool.query(query);
}

async function queryDateWithOperator(table: string, column: string, operator: string): Promise<QueryResult<any>> {
  const query = `SELECT max(${column}) as lastDate FROM ${table}
