...
class DriverDataDumpsController {
  ...
  public async getDriverDataDumps(request: Request, response: Response) {
    ...
    const lastDate = await driverDataDumpsAccessLayer.getLastDate(request.query.operator as string);
    validateEtag(request, lastDate);

    const stream = await driverDataDumpsAccessLayer.getStream(request.query.operator as string);
