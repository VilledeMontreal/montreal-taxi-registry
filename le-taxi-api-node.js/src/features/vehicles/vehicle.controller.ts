...
class VehiclesController {
  ...
  public async upsertVehicles(request: Request, response: Response) {
    ...
    const upsertedVehicle = await vehicleDataAccessLayer.upsertVehicle(vehicleRequestDto, request.userModel);
    const vehicleResponseDto = await vehicleDataAccessLayer.getVehicleById(upsertedVehicle.entityId as string);
  ...
  public async getVehicles(request: Request, response: Response) {
    ...
    if (vehicleId) {
      const vehicle = await vehicleDataAccessLayer.getVehicleById(vehicleId, operator);
