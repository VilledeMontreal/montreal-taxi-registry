...
class GtfsFeedController {
  ...
  public async getGtfsFeed(request: Request, response: Response) {
    ...
    zip.file('vehicle_categories.txt', gtfsFeedGenerator.getVehicleCategories(context));

    const stream = zip.generateNodeStream({ type: 'nodebuffer', streamFiles: true });
    response.setHeader('Content-Type', 'application/zip');
    response.setHeader('Content-disposition', 'attachment; filename=gtfs_feed.zip');
