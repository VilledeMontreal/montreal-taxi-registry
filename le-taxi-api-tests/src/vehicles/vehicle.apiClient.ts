...
export async function createVehicle(apiKey?: string, dto?: (x: IVehicle) => void): Promise<IVehicle> {
...
export async function postVehicle(dto: IVehicle, apiKey?: string) {
  ...
  const response = await postTaxiRegistry('/api/vehicles/', dto, apiKey, defaultApiKey);
  return response;
}
