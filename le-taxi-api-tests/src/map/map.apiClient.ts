...
export async function getTaxiOperators(apiKey?: string) {
  ...
  return await getTaxiRegistry('/api/legacy-web/taxi-operators', apiKey, defaultApiKey);
}
...
export async function getTaxiSearch(licencePlate?: string, professionalLicence?: string, apiKey?: string) {
  const defaultApiKey = await getImmutableUserApiKey(UserRole.Inspector);
