...
export async function invalidTaxiPositionSnapShotsTests(): Promise<void> {
  ...
  it("should return error when operator sends a taxi that doesnt exist.", async () => {
    const [dtoTaxiPositionSnapShot, apiKey] =
      await getValidTaxiPositionSnapshotDtoAndApikey(UserRole.Operator, 3);
  ...
  it("should return error when operator sends positions for a taxi that doesnt belong to him.", async () => {
    const [dtoTaxiPositionSnapShotA] =
      await getValidTaxiPositionSnapshotDtoAndApikey(UserRole.Operator);
    const [dtoTaxiPositionSnapShotB, operatorB] =
      await getValidTaxiPositionSnapshotDtoAndApikey(UserRole.Operator);

    dtoTaxiPositionSnapShotA.items[0].operator =
      dtoTaxiPositionSnapShotB.items[0].operator;
  ...
  it("should return error when lat/lon is invalid.", async () => {
    const [dtoTaxiPositionSnapShot, apiKey] =
      await getValidTaxiPositionSnapshotDtoAndApikey(UserRole.Operator, 7);
  ...
  it("should return error when lat/lon is out of bounds.", async () => {
    const [dtoTaxiPositionSnapShot, apiKey] =
      await getValidTaxiPositionSnapshotDtoAndApikey(UserRole.Operator, 2);
  ...
  it("should return error when azimuth is out of bounds.", async () => {
    const [dtoTaxiPositionSnapShot, apiKey] =
      await getValidTaxiPositionSnapshotDtoAndApikey(UserRole.Operator, 4);
...
function testCreateTaxiPositionSnapShotsUserAccessInvalid(role: UserRole) {
  it(`User with role ${UserRole[role]} should not be able to send taxi position snapshot`, async () => {
    ...
    const [dtoTaxiPositionSnapShot] =
      await getValidTaxiPositionSnapshotDtoAndApikey(UserRole.Operator);
    await shouldThrow(
      async () =>
        await postTaxiPositionSnapshots(dtoTaxiPositionSnapShot, apiKey),
      (err) => {
