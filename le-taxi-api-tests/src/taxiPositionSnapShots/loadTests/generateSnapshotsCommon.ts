...
export function beforeRequestForTests(requestParams: any, context: any, ee: any, next: any) {
  const sliced = sharedStateForTests.slice(0, configs.loadTesting.snapshots.numberOfOperators);
...
function generateTaxiPositionSnapshotsPayload(currentOperatorShareState: any): any {
  const points = generatePointsForLoadTest(currentOperatorShareState.taxi.length);
