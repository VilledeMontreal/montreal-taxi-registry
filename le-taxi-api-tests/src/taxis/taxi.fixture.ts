...
import { copyAdsOwnerTemplate, copyAdsPermitTemplate } from '../ads/adsDto.template';
...
export async function setupNewTaxiId(dtoTaxi: ITaxi, apiKey?: string): Promise<any> {
  const response = await setupNewTaxi(dtoTaxi, apiKey);
  return response.body.data[0].id;
}

export async function setupNewTaxi(dtoTaxi: ITaxi, apiKey?: string): Promise<any> {
  const [driver, vehicle, ads] = await Promise.all([createDriver(apiKey), createVehicle(apiKey), createAds(apiKey)]);
  return createTaxi({ ads, driver, vehicle, taxi: dtoTaxi }, apiKey);
}

export async function setupNewCustomTaxi(specialNeedVehicle: boolean = false, type: string = 'sedan', apiKey?: string) {
...
async function createTaxi(triplet: { ads: any; driver: any; vehicle: any; taxi: ITaxi }, apiKey?: string) {
...
async function createCustomVehicle(specialNeedVehicle: boolean = false, type: string = 'sedan', apiKey?: string) {
...
export async function initTaxiReferencesWithOwnerSemanticForADS(dtoTaxi: ITaxi, apiKey?: string) {
  await Promise.all([
    postDriver(
      copyDriverTemplate(x => {
        x.data[0].departement.numero = dtoTaxi.data[0].driver.departement;
        x.data[0].professional_licence = dtoTaxi.data[0].driver.professional_licence;
      }),
      apiKey
    ),
    postVehicle(
...
export async function initTaxiReferencesWithPermitSemanticForADS(dtoTaxi: ITaxi, apiKey?: string) {
  await Promise.all([
    postDriver(
      copyDriverTemplate(x => {
        x.data[0].departement.numero = dtoTaxi.data[0].driver.departement;
