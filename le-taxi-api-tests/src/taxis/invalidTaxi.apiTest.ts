...
import { initTaxiReferencesWithOwnerSemanticForADS, setupNewTaxi } from './taxi.fixture';
...
export async function invalidTaxiTests(): Promise<void> {
  ...
  it('Should return error with ads.insee not found', async () => {
    ...
    await shouldThrow(
      ...
      err => {
        assert.strictEqual(err.status, StatusCodes.BAD_REQUEST);
        assert.include(err.response.body.error.message, 'Unable to find a permit with insee');
      }
    );
  });
  ...
  it('Should return error with vehicle.licence_plate not found', async () => {
    ...
    await shouldThrow(
      ...
      err => {
        assert.strictEqual(err.status, StatusCodes.BAD_REQUEST);
        assert.include(err.response.body.error.message, 'Unable to find a vehicle with licence plate');
      }
    );
  });
  ...
  it('Should return error with driver.departement not found', async () => {
    ...
    await shouldThrow(
      ...
      err => {
        assert.strictEqual(err.status, StatusCodes.BAD_REQUEST);
        assert.include(err.response.body.error.message, 'Unable to find a driver with department');
      }
    );
  });
  ...
  it('GET. Should return error with aperator disallowed', async () => {
    ...
    await shouldThrow(
      ...
      err => {
        assert.strictEqual(err.status, StatusCodes.BAD_REQUEST);
        assert.include(err.response.body.error.message, 'Unable to find a taxi with id');
  ...
  it('Should NOT allow to mix legacy licence plate with ADS', async () => {
    ...
    await initTaxiReferencesWithOwnerSemanticForADS(attempt1ToMixLegacyLicencePlateForNewTaxi, apiKeyValidUser);
    await shouldThrow(
    ...
    await initTaxiReferencesWithOwnerSemanticForADS(attempt2ToMixLegacyLicencePlateForNewTaxi, apiKeyValidUser);
    await shouldThrow(
