...
import { getProfessionalLicence, postDriver } from '../drivers/driver.apiClient';
...
import { copyPutTaxiRequestTemplate, copyTaxiTemplate } from './taxisDto.template';
...
export async function crudTaxiTests(): Promise<void> {
  ...
  it('Cannot alter the driver of another operator', async () => {
    ...
    assert.strictEqual(canCreateMine.body.data[0].id, canUpdateMine.body.data[0].id);
    ...
    assert.strictEqual(cannotUpdateYours.body.data[0].id, butStillCanUpdateMine.body.data[0].id);
    assert.notEqual(canCreateMine.body.data[0].id, cannotUpdateYours.body.data[0].id);
  });
  ...
  it('POST Attribute status is ignored', async () => {
    ...
    const dtoUpdate = copyTaxiTemplate(x => {
      ...
      item.driver.professional_licence = responseItem.driver.professional_licence;
      item.status = 'free';
    });
  ...
  it('Returns valid characteristics', async () => {
    ...
    const taxiDtoUpdate = copyTaxiTemplate(x => {
      ...
      item.driver.professional_licence = taxiResponseItem.driver.professional_licence;
    });
    ...
    assert.strictEqual(taxiResponseItem.vehicle.constructor, 'constructor-updated');
    ...
    assert.include(taxiResponseItem.vehicle.characteristics, 'credit_card_accepted');
  ...
  it('PUT Attribute status is ignored', async () => {
    ...
    const putDto1 = copyPutTaxiRequestTemplate(x => (x.data[0].status = 'free'));
    await putTaxi(putDto1, taxiId);
    const getResponse1 = await getTaxiById(taxiId);
    assert.notEqual(getResponse1.body.data[0].status, 'free');

    const putDto2 = copyPutTaxiRequestTemplate(x => (x.data[0].status = 'oncoming'));
  ...
  it('Should be able to create duplicate taxi for same operator but different department', async () => {
    ...
    await initTaxiReferencesWithPermitSemanticForADS(firstDeptWithLicenseReuseDto);
    await initTaxiReferencesWithPermitSemanticForADS(secondDeptWithLicenseReuseDto);

    const canUseLicenseFor1stTime = await postTaxi(firstDeptWithLicenseReuseDto);
    const canReuseLicenseFor2ndTime = await postTaxi(secondDeptWithLicenseReuseDto);
    ...
    assert.notStrictEqual(canUseLicenseFor1stTime.body.data[0].id, canReuseLicenseFor2ndTime.body.data[0].id);
  });
  ...
  it('Should be able to create duplicate taxi for same operator but different zones', async () => {
    ...
    await initTaxiReferencesWithPermitSemanticForADS(firstZoneWithLicenseReuseDto);
    await initTaxiReferencesWithOwnerSemanticForADS(secondZoneWithLicenseReuseDto);
    ...
    assert.notStrictEqual(canUseForZone1.body.data[0].id, canReuseForZone2.body.data[0].id);
  });
